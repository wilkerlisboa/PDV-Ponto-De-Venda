...
def calcular_pagamento():
    try:
        ...
        valor_por_unidade = float(valor_por_unidade.get())

        valor_pagar = largura * comprimento * valor_por_unidade * quantidade_valor

        # Se houver acabamento e valor de acabamento, adicione ao valor a pagar
        if acabamento.get() == "Sim" and valor_acabamento_entry is not None:
...
def adicionar_ao_carrinho():
    ...
    valor = label_total.cget("text").split(":")[1].split(" -")[0].strip()
    quantidade_produto = quantidade.get()
    acabamento_produto = acabamento.get()
    ...
    # Verifica se a variável está definida antes de tentar deletar
    if valor_acabamento_entry is not None:
        valor_acabamento_entry.delete(0, tk.END)  # Limpa o campo de valor de acabamento
...
# Variável para armazenar o valor por unidade
valor_por_unidade = tk.StringVar()

# Variável para armazenar o valor total
valor_total = tk.StringVar()

# Inicialização da variável para o valor de acabamento
valor_acabamento_entry = None
...
frame_logo.grid(row=0, column=0, pady=10)  # Centraliza a logo
...
label_logo_fundo.grid(row=0, column=0, pady=10)  # Centraliza a logo

# Frame para os campos de produto
frame_produto = tk.Frame(frame_principal)
frame_produto.grid(row=0, column=1, pady=10, sticky="nsew")  # Adiciona sticky="nsew" para centralizar

# Campos para adicionar produto
label_produto = tk.Label(frame_produto, text="Produto:")
label_produto.grid(row=0, column=0, pady=5, sticky="w")
